...
public class EmployeeService
{
    ...
    public async Task UpdateEmployeeAsync(Employee employee)
    {
        ...
        if (existingEmployee == null){
            throw new AppException($"Employee with ID {employee.Employeeid} not found", ErrorCodes.NotFound);
        }   
        await ValidateEmployeeAsync(employee);
    ...
    public async Task UpdateEmployeeShiftAsync(decimal employeeId, decimal shiftId)
    {
        ...
        if (employee == null)
            throw new AppException($"Employee with ID {employeeId} not found", ErrorCodes.NotFound);

        var shift = await _shiftRepository.GetByIdAsync(shiftId);
        if (shift == null)
            throw new AppException($"Shift with ID {shiftId} not found", ErrorCodes.NotFound);
    ...
    public async Task AddEmployeeSkillAsync(decimal employeeId, string skill)
    {
        ...
        if (string.IsNullOrEmpty(skill))
            throw new AppException("Skill cannot be empty", ErrorCodes.ValidationError);
    ...
    private async Task ValidateEmployeeAsync(Employee employee)
    {
        ...
        if (employee.Shiftid.HasValue)
        {
            ...
            if (shift == null)
                throw new AppException($"Shift with ID {employee.Shiftid} not found", ErrorCodes.NotFound);
        }

        // 필수 필드 검증
        if (string.IsNullOrEmpty(employee.Name))
            throw new AppException("Employee name is required", ErrorCodes.ValidationError);

        if (employee.Hourlyrate < 0)
            throw new AppException("Hourly rate cannot be negative", ErrorCodes.ValidationError);
        ...
        if (!string.IsNullOrEmpty(employee.Role) && !validRoles.Contains(employee.Role))
            throw new AppException($"Invalid role: {employee.Role}. Valid roles are: {string.Join(", ", validRoles)}", ErrorCodes.ValidationError);
    }
}
